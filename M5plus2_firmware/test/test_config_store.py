...
CONFIG_SIZE = 31       # [name 24B][wheel 1B][rate 2B LE][fw 3B][sound 1B]
NAME_MAX_LEN = 24      # board name max bytes (fixed-width, null-padded)
NVS_NAMESPACE = "wacfg"
...
def pack_config(name, wheel_id, rate_hz, fw_major, fw_minor, fw_patch,
                beep_enabled=True):
    """Pack the v1.6 Config characteristic."""
    ...
    return name_buf + struct.pack('<BHB', wheel_id, rate_hz, fw_major) + \
           struct.pack('<BBB', fw_minor, fw_patch, 1 if beep_enabled else 0)
...
def parse_config(buf):
    ...
    wheel_id = buf[24]
    rate_hz = struct.unpack('<H', buf[25:27])[0]
    fw_major = buf[27]
    fw_minor = buf[28]
    fw_patch = buf[29]
    beep_enabled = buf[30] == 1
    return (name, wheel_id, rate_hz, fw_major, fw_minor, fw_patch, beep_enabled)
...
class TestPackConfig(unittest.TestCase):
    ...
    def test_wheel_id_field(self):
        ...
        self.assertEqual(buf[24], WHEEL_RIGHT)

    def test_rate_field_little_endian(self):
        buf = pack_config("Test", WHEEL_LEFT, 200, 0, 0, 0)
        rate = struct.unpack('<H', buf[25:27])[0]
        self.assertEqual(rate, 200)

    def test_fw_version_fields(self):
        buf = pack_config("Test", WHEEL_LEFT, 50, 1, 2, 3)
        self.assertEqual(buf[27], 1)  # major
        self.assertEqual(buf[28], 2)  # minor
        self.assertEqual(buf[29], 3)  # patch

    def test_countdown_sound_field(self):
        self.assertEqual(pack_config("Test", WHEEL_LEFT, 100, 1, 6, 0, True)[30], 1)
        self.assertEqual(pack_config("Test", WHEEL_LEFT, 100, 1, 6, 0, False)[30], 0)
...
class TestParseConfig(unittest.TestCase):
    ...
    def test_roundtrip(self):
        original = ("WheelAthlete-R", WHEEL_RIGHT, 200, 0, 2, 1, True)
        buf = pack_config(*original)
        parsed = parse_config(buf)
        self.assertEqual(parsed, original)

    def test_short_name_roundtrip(self):
        original = ("L1", WHEEL_LEFT, 50, 1, 0, 0, False)
        buf = pack_config(*original)
        parsed = parse_config(buf)
        self.assertEqual(parsed, original)
    ...
    def test_empty_name(self):
        ...
        name, wheel, rate, _, _, _, _ = parse_config(buf)
        self.assertEqual(name, "")

    def test_truncated_buffer_raises(self):
        with self.assertRaises(ValueError):
            parse_config(b'\x00' * 30)
...
class TestSanitizeName(unittest.TestCase):
    ...
    def test_short_name_padded(self):
        ...
        self.assertEqual(buf[5:], b'\x00' * 19)

    def test_exact_24_chars(self):
        name = "A" * 24
        buf = sanitize_name(name)
        self.assertEqual(len(buf), NAME_MAX_LEN)
        self.assertEqual(buf, b'A' * 24)
