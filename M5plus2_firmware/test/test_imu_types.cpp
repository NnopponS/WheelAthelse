...
void test_target_batch_count_100hz_mtu_247(void) {
    // Keep a slow second Android link at or below 10 notifications/second.
    TEST_ASSERT_EQUAL_UINT8(10, targetBatchCount(247, 100));
}

void test_target_batch_count_200hz_mtu_247(void) {
    // 200 Hz uses the full MTU capacity (12 samples) to minimize notify rate.
    TEST_ASSERT_EQUAL_UINT8(12, targetBatchCount(247, 200));
}

void test_target_batch_count_50hz_mtu_247(void) {
    TEST_ASSERT_EQUAL_UINT8(5, targetBatchCount(247, 50));
}
...
void test_target_batch_count_invalid_rate_uses_default(void) {
    TEST_ASSERT_EQUAL_UINT8(10, targetBatchCount(247, 75));
}

void test_notification_retry_backoff_is_bounded(void) {
    TEST_ASSERT_EQUAL_UINT32(0, notificationRetryDelayMs(0));
    TEST_ASSERT_EQUAL_UINT32(10, notificationRetryDelayMs(1));
    TEST_ASSERT_EQUAL_UINT32(20, notificationRetryDelayMs(2));
    TEST_ASSERT_EQUAL_UINT32(40, notificationRetryDelayMs(3));
    TEST_ASSERT_EQUAL_UINT32(80, notificationRetryDelayMs(4));
    TEST_ASSERT_EQUAL_UINT32(100, notificationRetryDelayMs(5));
    TEST_ASSERT_EQUAL_UINT32(100, notificationRetryDelayMs(20));
}

void test_notification_retry_due_handles_millis_wrap(void) {
    TEST_ASSERT_FALSE(notificationRetryDue(99u, 100u));
    TEST_ASSERT_TRUE(notificationRetryDue(100u, 100u));
    TEST_ASSERT_TRUE(notificationRetryDue(101u, 100u));
    TEST_ASSERT_FALSE(notificationRetryDue(0xFFFFFFF0u, 20u));
    TEST_ASSERT_TRUE(notificationRetryDue(20u, 20u));
}
...
int main() {
    ...
    RUN_TEST(test_notification_retry_backoff_is_bounded);
    RUN_TEST(test_notification_retry_due_handles_millis_wrap);
