...
void BleService::onConnect(uint16_t conn_handle, uint16_t conn_interval_units) {
    ...
    // Match the XIAO/PC lifecycle contract: a new central connection always
    // starts from an idle acquisition epoch with no stale samples or sequence.
    if (imu().running()) {
        imu().stop();
    }
    ...
    ble().replay_pending_ = false;
    ble().pending_count_ = 0;
    ble().replay_head_ = 0;
    ble().replay_count_ = 0;
...
void BleService::handleStart(uint32_t target_start_us) {
    ...
    // Reset immediately when START is accepted, not only when the scheduled
    // T0 fires. Stop an unexpected local/previous producer first so the FIFO
    // task cannot race the queue reset. Normal PC lifecycle is already idle.
    if (imu().running()) {
        imu().stop();
    }
    imu().resetQueueAndSeq();
