...
void ImuReader::resetQueueAndSeq() {
    ...
    // Drop any stale wakeup left by the previous acquisition epoch.
...
void ImuReader::handleFifoFault(uint16_t fifo_bytes) {
    ...
    fifo_dropped_sample_count_ += estimatedDroppedSamplesFromFifoBytes(fifo_bytes);
    last_fifo_depth_ = fifo_bytes;
    ...
    if (fifo_overflow_count_ <= 5 || now_ms - last_fifo_fault_log_ms_ >= 1000) {
        Serial.printf("[IMU] FIFO fault bytes=%u remainder=%u drops=%lu faults=%lu\n",
                      ...
                      static_cast<unsigned long>(fifo_dropped_sample_count_),
