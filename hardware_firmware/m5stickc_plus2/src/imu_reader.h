...
public:
    ...
    // Clear buffered samples and start a fresh sequence/counter epoch.
    // Used by BLE connect/START so the PC never inherits stale local data.
    ...
    uint32_t    queueDropCount() const { return queue_drop_count_; }
    uint32_t    fifoDroppedSampleCount() const { return fifo_dropped_sample_count_; }
    uint32_t    dropCount()   const {
        return queue_drop_count_ + fifo_dropped_sample_count_;
    }
...
private:
    ...
    uint32_t    queue_drop_count_ = 0;
    uint32_t    fifo_dropped_sample_count_ = 0;
