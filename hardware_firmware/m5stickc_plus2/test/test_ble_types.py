...
def pack_countdown_cue(index, total, duration_ms):
    return struct.pack('<BBBH', EVENT_COUNTDOWN_CUE, index, total, duration_ms)
...
class TestSyncEvents(unittest.TestCase):
    ...
    def test_countdown_cue(self):
        buf = pack_countdown_cue(3, 4, 500)
        self.assertEqual(buf, bytes([EVENT_COUNTDOWN_CUE, 3, 4, 0xF4, 0x01]))

    def test_header_exposes_countdown_cue_event(self):
        source = (Path(__file__).parents[1] / "src" / "ble_types.h").read_text(
            encoding="utf-8"
        )
        self.assertIn("CountdownCue", source)
        self.assertIn("0x31", source)
        self.assertIn("packCountdownCue", source)
...
class TestProtocol13ReplayContract(unittest.TestCase):
    def test_replay_command_and_result_are_declared(self):
        source = Path(__file__).parents[1].joinpath('src', 'ble_types.h').read_text()
        self.assertIn('ReplayRange= 0x0A', source)
        self.assertIn('AcqHealth    = 0x60', source)
        self.assertIn('ReplayResult = 0x61', source)
        self.assertIn('packReplayResult', source)


class TestProtocol16AcqHealthContract(unittest.TestCase):
    def test_health_layout_is_exactly_28_bytes(self):
        payload = struct.pack('<BBIIIIHII', 0x60, 2, 1234, 1200, 3, 4, 7, 8, 9)
        self.assertEqual(len(payload), 28)
        self.assertEqual(payload[0], 0x60)
        state, produced, notified, drops, failures, depth, fifo_faults, fifo_drops = struct.unpack(
            '<BIIIIHII', payload[1:]
        )
        self.assertEqual(
            (state, produced, notified, drops, failures, depth, fifo_faults, fifo_drops),
            (2, 1234, 1200, 3, 4, 7, 8, 9),
        )

    def test_header_exposes_health_packer_and_static_size(self):
        source = Path(__file__).parents[1].joinpath('src', 'ble_types.h').read_text()
        self.assertIn('ACQ_HEALTH_SIZE    = 28', source)
        self.assertIn('packAcqHealth', source)

    def test_produced_count_includes_samples_lost_before_queueing(self):
        source = Path(__file__).parents[1].joinpath('src', 'ble_service.cpp').read_text()
        self.assertIn('imu().sampleCount() + imu().queueDropCount() +', source)
        self.assertIn('imu().fifoDroppedSampleCount()', source)

    def test_health_separates_sample_queue_drops_from_imu_fifo_faults(self):
        root = Path(__file__).parents[1] / 'src'
        header = (root / 'imu_reader.h').read_text()
        source = (root / 'ble_service.cpp').read_text()
        self.assertIn('queueDropCount()', header)
        self.assertIn('fifoDroppedSampleCount()', header)
        self.assertIn('imu().queueDropCount()', source)
        self.assertIn('imu().fifoOverflowCount()', source)
        self.assertIn('imu().fifoDroppedSampleCount()', source)

    def test_final_health_is_available_in_serial_acceptance_logs(self):
        source = Path(__file__).parents[1].joinpath('src', 'ble_service.cpp').read_text()
        self.assertIn('[ACQ] FINAL', source)
        self.assertIn('queue_drops=', source)
        self.assertIn('fifo_faults=', source)
        self.assertIn('fifo_drops=', source)
        self.assertIn('transport_failures=', source)

    def test_batch_latency_and_history_are_bounded(self):
        source = Path(__file__).parents[1].joinpath('src', 'ble_service.h').read_text()
        self.assertIn('BATCH_MAX_LATENCY_MS = 100', source)
        self.assertIn('REPLAY_HISTORY_SIZE = 512', source)
