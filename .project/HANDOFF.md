...
The tested source commit `eab67ca23edfa078cd866008e2e1f1c81648b08c` is pushed to `main` and `release/main-v1.8.4`, with exact tag `v1.8.4`. GitHub Actions run 35913784312 failed at `Import required Windows signing certificate`; publishing was skipped. There is no public v1.8.4 setup.exe, portable ZIP, checksums or stable update manifest, and the latest GitHub release remains v1.8.3. The certificate-import log shows empty `SIGN_PFX_BASE64` and `SIGN_PFX_PASSWORD` inputs and aborts because signing secrets and release variables are required. GitHub settings confirm the required names are absent: secrets `WHEELATHLETE_SIGN_PFX_BASE64` and `WHEELATHLETE_SIGN_PFX_PASSWORD`; variables `WHEELATHLETE_TIMESTAMP_URL` and `WHEELATHLETE_SIGN_CERT_SUBJECT`. Add those signing values, then rerun the workflow for exact tag `v1.8.4` and confirm the signed artifacts and stable update manifest before publishing. Never use unsigned assets.
