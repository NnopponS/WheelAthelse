...
- Window close is blocked with a wait message during Results or Model CSV export; trajectory and imported-CSV exports publish only after the temporary copy is complete. Recording close finalizes the journal and waits for daemon shutdown acknowledgement. Cleanup options are opt-in, confirmed, and limited to managed WheelAthlete locations.
- The v1.8.4 workflow rejects manual dispatch and tag deletions, skips Windows/Android artifact builds, and publishes release notes only on a non-deleted tag push. The old tag/release branch at `eab67ca23edfa078cd866008e2e1f1c81648b08c` was replaced with tested commit `80cae399f7dd5909c93e2dd6acdc927871518c8b`; the source-only [GitHub Release](https://github.com/NnopponS/WheelAthelse/releases/tag/v1.8.4) and its tag-push workflow run both succeeded.
- No mobile source/assets, `BiWheel3D/`, firmware queue-backlog fix, or locked paper-test cohort change is included.

Software acceptance: Windows suite **243 passed, 6 skipped**; M5 host tests **147 passed**; XIAO host tests **19 passed**; PlatformIO builds succeeded for M5/XIAO left, right, and center; Python compile check and working-tree/staged project hygiene passed. The release workflow succeeded with no binary assets. Inno Setup is not installed here, so packaging-layout tests passed but the installer script was not compiled. These checks do not prove physical or cross-computer BLE acceptance.
