...
Verification passed: 231 Windows tests, 5 skipped; project hygiene; 21 XIAO host tests; left/right/center builds. All three connected boards were flashed and reported firmware 1.8.3 with L/R/C roles intact. The stationary 100 Hz stress capture ran 1080.326 seconds and finalized GOOD: 331,815 journal samples (L 110,601; R 110,608; C 110,606), zero sequence gaps, device queue/FIFO drops, transport failures, host overflows, malformed packets, or QC reasons; writer overflow was zero and queue high-water was 29. This is acquisition/RF stress evidence, not physical motion or model-accuracy evidence. The v1.8.4 commit is pushed to `main`, `release/main-v1.8.4` and tag `v1.8.4`. GitHub Actions run 35913784312 failed at `Import required Windows signing certificate`; publish was skipped, so v1.8.4 has no public setup.exe, portable ZIP, checksums or update manifest. GitHub's latest release remains v1.8.3. The certificate-import log shows the signing PFX and password inputs were empty; GitHub settings confirm the required certificate/password secrets and timestamp/publisher variables are absent. Configure the signing values and rerun the exact-tag workflow; do not publish unsigned assets.
