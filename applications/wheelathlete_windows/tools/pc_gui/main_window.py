...
class MainWindow(QMainWindow):
    def __init__(self, controller: BaseController, *, demo: bool = False) -> None:
        ...
        self._update_update_ui(self.update_controller.state)
    ...
    def _update_update_ui(self, state: UpdateViewState) -> None:
        if state.status == "checking":
            self.update_button.setText("Checking updates…")
            self.update_button.setEnabled(False)
        elif state.status == "downloading":
            self.update_button.setText(f"Downloading {state.progress_percent}%")
            self.update_button.setEnabled(False)
            ...
            self.update_button.setText(f"v{state.current_version} · Up to date")
            self.update_button.setEnabled(True)
        else:
            self.update_button.setText("Check updates")
            self.update_button.setEnabled(True)
        self.update_button.setToolTip(state.message or "Check for software updates")
