...
def fit_scale_for_view(*, x_span: float, y_span: float, width: int, height: int) -> float:
    """Fit projected metre spans into the plot area using one scale for both axes."""
    plot_width = max(1, width - 60)
    plot_height = max(1, height - 125)
    return 0.9 * min(
        plot_width / max(float(x_span), 0.1),
        plot_height / max(float(y_span), 0.1),
    )
...
class Trajectory3DView(QWidget):
    ...
    def view_center(self) -> tuple[float, float, float] | None:
        ...
        return tuple(
            (min(p[i] for p in self._source_points) + max(p[i] for p in self._source_points)) / 2
            for i in range(3)
        )
    ...
    def fitted_scale(self, width: int, height: int) -> float:
        ...
        if center is None:
            return 0.0
        projected = [self._project(point, center) for point in self._source_points]
        x_span = max(point[0] for point in projected) - min(point[0] for point in projected)
        y_span = max(point[1] for point in projected) - min(point[1] for point in projected)
        extent = max(
            max(point[axis] for point in self._source_points)
            - min(point[axis] for point in self._source_points)
            for axis in range(3)
        )
        grid_half = max(extent * 0.4, 0.5)
        ground = min(point[2] for point in self._source_points)
        grid = [
            (center[0] + offset, center[1] + sign * grid_half, ground)
            for offset in (-grid_half, grid_half)
            for sign in (-1, 1)
        ]
        projected.extend(self._project(point, center) for point in grid)
        x_span = max(point[0] for point in projected) - min(point[0] for point in projected)
        y_span = max(point[1] for point in projected) - min(point[1] for point in projected)
        return fit_scale_for_view(
            x_span=x_span, y_span=y_span, width=width, height=height
        )
    ...
    def paintEvent(self, event) -> None:
        ...
        extent = max(max(p[i] for p in self._source_points) - min(p[i] for p in self._source_points) for i in range(3))
        grid_half = max(extent * 0.4, 0.5)
        ...
        all_projected = [self._project(p, center) for p in self._source_points]
        all_projected.extend(self._project(p, center) for line in grid for p in line)
        ...
        scale = fit_scale_for_view(
            x_span=max_x - min_x,
            y_span=max_y - min_y,
            width=self.width(),
            height=self.height(),
        ) * self.zoom
        plot_top, plot_bottom = 70.0, float(self.height() - 32)
        origin = QPointF(
            self.width() / 2 - (min_x + max_x) * scale / 2,
            (plot_top + plot_bottom) / 2 + (min_y + max_y) * scale / 2,
        )
