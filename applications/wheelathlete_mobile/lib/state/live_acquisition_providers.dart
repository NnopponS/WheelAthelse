...
/// Mirrors the Windows acquisition service live-preview defaults.
final liveSyncCountProvider = Provider<int>((ref) => 5);
final liveStartLeadTimeProvider = Provider<Duration>(
...
class LiveAcquisitionNotifier extends Notifier<LiveAcquisitionState> {
  ...
  Future<void> start() async {
    ...
    try {
      ...
      try {
        // Subscribe downstream before firmware START so the first notification
        // cannot race CCCD setup.
        await Future.wait(
        ...
        // PC parity: each ping must receive its matching response before the
        // next observation is issued. GATT writes remain serialized globally.
        final syncCount = ref.read(liveSyncCountProvider);
        for (final side in sides) {
          await sync.synchronize(side, count: syncCount);
        }

        // Every wheel receives a different device-local target derived from
        // exactly one common phone-monotonic T0.
        final lead = ref.read(liveStartLeadTimeProvider);
        final commonPhoneStartUs = sync.nowMonotonicUs + lead.inMicroseconds;
        for (final side in sides) {
          await sync.sendStart(
            side,
            targetStartUs: sync.targetDeviceUs(side, commonPhoneStartUs),
          );
        }

        final ackTimeout = lead + ref.read(liveAckTimeoutProvider);
        final waits = {
          for (final side in sides)
  ...
  Future<void> stop() async {
    ...
    try {
      ...
      // Stop replay/control traffic before STOP so the write cannot queue
      // behind recovery work on a congested Android BLE stack.
  ...
  Future<void> _rollback(Set<WheelSide> sides) async {
    ...
    final hub = ref.read(imuSampleHubProvider);
    await Future.wait(sides.map(hub.suspendReplay));
    // Match the PC fail-closed behavior: best-effort STOP every selected board
    // before tearing down local consumers.
    for (final side in sides) {
      try {
        await sync.sendStopWithRetry(side);
      } on Object {
        // Disconnect path remains available to the connection manager/user.
      }
    }
    final imu = ref.read(imuStreamProvider.notifier);
