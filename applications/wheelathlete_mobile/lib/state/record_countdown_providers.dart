...
class RecordCountdownNotifier extends Notifier<RecordCountdownState> {
  ...
  Future<void> start(SessionConfig config) async {
    ...
    // 1. PC-parity synchronization. Each wheel completes one round trip before
    // the next ping is sent; every observation uses the T1/T3 midpoint and the
    // fitted model keeps the lowest-RTT half.
    ...
    try {
      for (final side in connectedSides) {
        ...
        await sync.synchronize(
          side,
          count: kSyncBurstCount,
          interPingDelay: kSyncBurstInterval,
        );
      }
    } on Object catch (e) {
      ...
      state = RecordCountdownState(
        status: RecordCountdownStatus.error,
        error: 'Clock synchronization failed: $e',
      );
      return;
    }
    if (_cancelled || !ref.mounted) return;
    ...
    final leadUs = (tStartPhoneMs - nowPhoneMs) * 1000;
    final commonPhoneStartUs = sync.nowMonotonicUs + leadUs;
    final tAppRefUs = sync.tAppRefUs ?? commonPhoneStartUs;
    final tStartRelMs = (commonPhoneStartUs - tAppRefUs) ~/ 1000;
    ...
    // 4. Derive every board-local target from exactly one phone-monotonic T0.
    // Writes are intentionally serialized, as in the Windows app; write order
    // is not the synchronization mechanism.
    try {
      for (final side in connectedSides) {
        await _sendScheduledStart(side, commonPhoneStartUs);
      }
    } on Object catch (e) {
      for (final side in connectedSides) {
        try {
          ...
          // Best-effort rollback. Firmware also stops on disconnect.
        }
      }
      await ref.read(recordingProvider.notifier).disarmStreaming();
      if (!ref.mounted) return;
      state = RecordCountdownState(
        status: RecordCountdownStatus.error,
        error: 'Scheduled START failed: $e',
      );
      return;
    }
    if (_cancelled || !ref.mounted) return;
  ...
  Future<void> _sendScheduledStart(
    WheelSide side,
    int commonPhoneStartUs,
  ) async {
    final sync = ref.read(syncEngineProvider.notifier);
    ...
    final targetStartUs = sync.targetDeviceUs(side, commonPhoneStartUs);
