...
class RecordingWheelHealth {
  const RecordingWheelHealth({
    ...
    this.hostQueueHighWater = 0,
    this.hostQueueOverflowFaults = 0,
    this.malformedHostPackets = 0,
  ...
  final int hostQueueHighWater;
  final int hostQueueOverflowFaults;
  final int malformedHostPackets;
...
///
/// State machine:
/// - idle â†’ startRecording â†’ recording (streams IMU, buffers samples)
/// - recording â†’ stopRecording â†’ stopped (saves to storage, stops streams)
/// - stopped â†’ reset â†’ idle (clears state for next session)
///
/// The Mark Event function was removed in Phase 3 (D16) â€” IMU data is now
...
class RecordingNotifier extends Notifier<RecordingState> {
  ...
  ///
  /// Call this as early as possible â€” e.g. at the very start of the
  /// countdown flow, well before the scheduled START command is sent â€” so
  ...
  ///
  /// Previously, IMU streaming was only subscribed to *after* START_FIRED â€”
  /// i.e. after the firmware had already begun streaming â€” which raced the
  ...
  ///
  /// Idempotent â€” sides that are already armed (have an active subscription)
  /// are left untouched.
  ///
  ...
  Future<void> startRecording(SessionConfig config) async {
    ...
    // Normally a no-op â€” the countdown flow already calls [armStreaming]
    // well before this point â€” but kept here so startRecording remains safe
    ...
    _healthTimer = Timer.periodic(const Duration(milliseconds: 250), (_) {
      ...
      for (final side in _expectedSides) {
        ...
        final currentFailure = freshFirmwareHealth
            ...
            : null;
        if (failedSide == null &&
            recovery.notificationQueueOverflowFaults > 0) {
          failedSide = side;
          failureCause = AcquisitionFailureCause.hostIngestionOverflow;
        } else if (failedSide == null && recovery.malformedPackets > 0) {
          failedSide = side;
          failureCause = AcquisitionFailureCause.malformedHostPacket;
        } else if (currentFailure != null && failedSide == null) {
          failedSide = side;
        ...
        health[side] = RecordingWheelHealth(
          ...
          hostQueueHighWater: recovery.notificationQueueHighWater,
          hostQueueOverflowFaults: recovery.notificationQueueOverflowFaults,
          malformedHostPackets: recovery.malformedPackets,
  ...
  Future<void> _abortForAcquisitionHealth(
    ...
    final recovery = ref.read(imuSampleHubProvider).metrics(side);
    _forcedDegradation = switch (cause) {
      AcquisitionFailureCause.hostIngestionOverflow =>
        '${_sideLabel(side)} phone ingress queue overflow '
            '(high-water ${recovery.notificationQueueHighWater})',
      AcquisitionFailureCause.malformedHostPacket =>
        '${_sideLabel(side)} malformed IMU notification '
            '(count ${recovery.malformedPackets})',
    ...
    _continuousSyncTimer = Timer.periodic(_continuousSyncInterval, (_) {
      ...
      for (final side in WheelSide.values) {
        if (connState.bySide[side]!.status == ConnectionStatus.connected &&
          ...
          futures.add(sync.pingRoundTrip(side).then<void>((_) {}));
        }
      }
      unawaited(
  ...
  /// is zero â€” the test default) or deferred to the next interval tick. The
  ...
  Future<void> stopRecording() {
    ...
    final future = _stopRecordingImpl().whenComplete(() {
      _stopFuture = null;
    });
    _stopFuture = future;
    return future;
  ...
  Future<void> _stopRecordingImpl() async {
    ...
    // The BLE callback is intentionally decoupled from parsing. Before deciding
    // that a trial has no samples, drain every notification already accepted by
    // the bounded ingress worker. This is the mobile equivalent of the PC
    // acquisition engine's join() barrier.
    final ingressHub = ref.read(imuSampleHubProvider);
    final ingressSides = _subs.keys.toList(growable: false);
    await Future.wait(ingressSides.map(ingressHub.join));
    ...
    // STOP_FIRED means the firmware has drained its queue. Now also drain the
    // phone-side bounded ingress queue before taking QC metrics or saving.
    // No notification accepted before this barrier can be silently omitted.
    await Future.wait(stopSides.map(ingressHub.join));
    _flushEmit();
  ...
  String? _degradationReason({
    ...
    if (stopSides.any(
      (side) => recoveryMetrics[side]!.notificationQueueOverflowFaults > 0,
    )) {
      reasons.add('Phone notification ingress queue overflow');
    }
    if (stopSides.any((side) => recoveryMetrics[side]!.malformedPackets > 0)) {
      reasons.add('Malformed IMU notification received by phone');
    }
...
/// [RecordingNotifier].
///
/// Defaults to ~100 ms (10 Hz) in production â€” the recording UI only needs to
