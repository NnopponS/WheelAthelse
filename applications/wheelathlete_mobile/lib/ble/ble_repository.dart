...
class FakeBleRepository implements BleRepository {
  ...
  final Map<String, int> _fakeSyncSequence = {};
  final Map<String, int> _fakeLastStartUs = {};
  ...
  Future<void> writeControl(String deviceId, List<int> bytes) async {
    ...
    if (autoRespondToSync && bytes.length >= 5 && bytes[0] == 0x04) {
      final input = ByteData.sublistView(Uint8List.fromList(bytes));
      final tAppMs = input.getUint32(1, Endian.little);
      final deviceUs = (1000000 + tAppMs * 1000) & 0xFFFFFFFF;
      final seq = (_fakeSyncSequence[deviceId] ?? 0) + 1;
      _fakeSyncSequence[deviceId] = seq;
      final response = ByteData(13)
        ..setUint8(0, 0x00)
        ..setUint32(1, tAppMs, Endian.little)
        ..setUint32(5, deviceUs, Endian.little)
        ..setUint32(9, seq, Endian.little);
      scheduleMicrotask(
        () => _syncController(deviceId).add(response.buffer.asUint8List()),
      );
    }
    if (autoAcknowledgeControls && bytes.isNotEmpty) {
      if (bytes[0] == 0x01) {
        var firedUs = 0;
        if (bytes.length >= 5) {
          firedUs = ByteData.sublistView(
            Uint8List.fromList(bytes),
          ).getUint32(1, Endian.little);
        }
        _fakeLastStartUs[deviceId] = firedUs;
        final response = ByteData(13)
          ..setUint8(0, 0x30)
          ..setUint32(1, firedUs, Endian.little);
        scheduleMicrotask(
          () => _syncController(deviceId).add(response.buffer.asUint8List()),
        );
        ...
        final response = ByteData(9)
          ...
          ..setUint32(
            1,
            ((_fakeLastStartUs[deviceId] ?? 0) + 100000) & 0xFFFFFFFF,
            Endian.little,
          );
        scheduleMicrotask(
          () => _syncController(deviceId).add(response.buffer.asUint8List()),
