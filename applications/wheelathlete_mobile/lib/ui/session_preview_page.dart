  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return ListView(
      ...
      children: [
        ...
        const SizedBox(height: AppSpacing.md),
        _TrajectoryModelCard(source: source, meta: state.meta),
        const SizedBox(height: AppSpacing.md),
...
class _TrajectoryModelCard extends ConsumerWidget {
  const _TrajectoryModelCard({required this.source, required this.meta});
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final modelState = ref.watch(modelTrajectoryProvider(source));
    final theme = Theme.of(context);
    final result = modelState.result;
    final preprocess = modelState.preprocess;

    return Card(
      key: const Key('trajectoryModelCard'),
      child: Padding(
        padding: const EdgeInsets.all(AppSpacing.md),
        child: Column(
          ...
          children: [
            ...
            SizedBox(
              ...
              child: FilledButton.icon(
                key: const Key('generateTrajectoryButton'),
                onPressed: modelState.isRunning
                    ? null
                    : () => ref
            ...
            if (modelState.error case final error?) ...[
              const SizedBox(height: AppSpacing.sm),
              Container(
                key: const Key('trajectoryModelError'),
                width: double.infinity,
            ...
            if (result != null) ...[
              ...
              Row(
                children: [
                  ...
                  if (preprocess != null)
                    Text(
                      '${preprocess.modelStepCount} steps',
              ...
              Wrap(
                ...
                children: [
                  StatusBadge(
                    label: '${result.pathLengthM.toStringAsFixed(2)} m path',
                    icon: Icons.timeline_rounded,
                    dense: true,
                  ),
                  StatusBadge(
                    label: '${result.endpointM.toStringAsFixed(2)} m endpoint',
                  ...
                  if (preprocess != null)
                    StatusBadge(
                      label:
                          '${preprocess.durationSeconds.toStringAsFixed(1)} s model input',
