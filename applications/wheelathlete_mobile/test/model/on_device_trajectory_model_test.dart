...
TrajectoryPreprocessResult fixtureInput() {
  ...
  return TrajectoryPreprocessResult(
    ...
    durationSeconds: windows.length * trajectoryGroupSize / trajectoryTargetRawHz,
...
void main() {
  ...
  test('bundled ONNX runs offline and preserves experimental XY-only contract', () async {
    ...
    expect(analysis.metadata['model_sha256'], matches(RegExp(r'^[0-9a-f]{64}$')));
    expect(analysis.metadata['model_validation_status'], 'not_independently_validated');
    ...
    expect(analysis.samples[input.modelStepCount ~/ 2]['speed_mps'], isNotNull);
  }, timeout: const Timeout(Duration(minutes: 2)));

  test('disposed client rejects inference without opening native model', () async {
    final client = OnDeviceTrajectoryModelClient();
