...
Future<(FakeBleRepository, ProviderContainer)> _connectedLiveContainer({
  ...
  final ble = FakeBleRepository(
    devices: const [
      FakeDevice(id: 'L1', name: 'WheelAthlete-XIAO-L', rssi: -40),
...
void main() {
  ...
  test(
    'mixed XIAO and M5 v1.8.0 use scheduled non-zero START targets',
    () async {
      final (ble, container) = await _connectedLiveContainer(
        stopWriteFailuresFor: const {},
      );
      ...
      for (final deviceId in ['L1', 'R1']) {
        final starts = ble
            ...
            .where((bytes) => bytes.isNotEmpty && bytes.first == 0x01)
            .toList(growable: false);
        expect(starts, hasLength(1));
        final payload = ByteData.sublistView(Uint8List.fromList(starts.single));
        expect(
          payload.getUint32(1, Endian.little),
          greaterThan(0),
          reason: '$deviceId must use scheduled START, never START(0)',
        );
      }
