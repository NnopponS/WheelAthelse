...
class _FakeTrajectoryModelClient implements TrajectoryModelClient {
  int calls = 0;
  int? lastModelSteps;
  ...
  Future<TrajectoryResult> infer({
    ...
    calls++;
    lastModelSteps = input.modelStepCount;
    return const TrajectoryResult(
      modelLabel: 'Fake BiWheel3D M4',
      points: [
        TrajectoryPoint(0, 0),
        TrajectoryPoint(1, 0),
        TrajectoryPoint(1, 1),
...
void main() {
  ...
  testWidgets('on-device model generates and plots a 2D trajectory', (
    tester,
  ) async {
    final client = _FakeTrajectoryModelClient();
    final samples = mixedSamples(240);
    final meta = _meta(durationMs: 2400, sampleCount: samples.length);
    final source = InMemoryPreviewSource(meta: meta, samples: samples);
    ...
    expect(find.text('Model trajectory'), findsOneWidget);
    expect(find.byKey(const Key('trajectoryEndpointField')), findsNothing);
    expect(find.text('On-device'), findsOneWidget);
    expect(find.byKey(const Key('generateTrajectoryButton')), findsOneWidget);

    await tester.tap(find.byKey(const Key('generateTrajectoryButton')));
    await tester.pumpAndSettle();

    expect(client.calls, 1);
    expect(client.lastModelSteps, greaterThanOrEqualTo(40));
    expect(find.text('Fake BiWheel3D M4'), findsOneWidget);
    expect(find.text('2.00 m path'), findsOneWidget);
    expect(find.text('1.41 m endpoint'), findsOneWidget);
