...
Branch: `feature/dual-imu-coaching-analysis`. This is an experimental feature branch, not a new stable release; the version matrix below remains unchanged. Windows now supports original-time trajectory/speed/yaw window review. Mobile supports the same review/export semantics for its retained XY-only model, with chair yaw and signed forward speed unavailable. No new independent accuracy or installed-device acceptance is claimed.
...
The Windows `MODEL` page now bundles the **BiWheel3D XY + Yaw current_best** planar runtime. PyTorch and external checkpoint files are no longer required. Source development needs NumPy:

```bat
python -m pip install -r applications\wheelathlete_windows\tools\pc_gui\requirements-model.txt
```

The packaged Windows GUI includes NumPy, the current-best calibration metadata, the minimal BiWheel3D runtime snapshot, and its MIT license. Analysis remains offline and isolated from acquisition; `.waj` remains the authoritative research record.
