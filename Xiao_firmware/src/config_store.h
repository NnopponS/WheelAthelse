#pragma once
// config_store.h — Board configuration cache + LittleFS-backed store
//
...
// ── ConfigStore — LittleFS-backed persistent config ──
class ConfigStore {
public:
    // Load config from LittleFS. Call before ble().begin().
    ...
    bool beepEnabled() const { return beep_enabled_; }
