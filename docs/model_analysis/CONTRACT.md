...
The source Model page can also discover the hash-matched Phase E `biwheel3d:sif_yaw_consensus` artifact from the local research lifecycle registry or browse its `model.json` directly. Readiness validates the local research runtime and artifact. It accepts finalized synchronized L/R/C recordings, not processed dual-hub NPZ windows; inference never uses C3D, condition or athlete labels. The adapter restores the frozen research preprocessing scale of 0.070 degrees per raw gyro count after the app's physical-to-count conversion, while the existing app v3 runtime keeps its own scale. A development-record replay matched frozen SIF XYZ within 1e-7 m. This bridge remains optional and does not change the application default or make the separate research repository an installed dependency.

The Model page's 3D view preserves model XYZ when available, supports mouse orbit and zoom, and labels XY-only paths as Z unavailable. Its 2D metrics remain planar; trajectory CSV writes an empty `z_m` cell for XY-only models. Exported images reflect the selected view. The display does not establish physical trajectory accuracy.
